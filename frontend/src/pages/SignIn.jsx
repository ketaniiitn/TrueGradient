...
const SignIn = () => {
  ...
  const handleBlur = (e) => {
    ...
    // re-run validation for this field only
    setFormErrors((prev) => {
      const next = { ...prev };
  ...
  // Map backend auth error to field-level errors after submit
  useEffect(() => {
    if (!error) return;
    if (error.toLowerCase().includes('invalid')) {
      setFormErrors((prev) => ({
        ...prev,
        username: prev.username || 'Check username',
        password: prev.password || 'Check password',
      }));
    }
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center px-4 py-6 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all shadow-sm ${
                showUsernameError ? 'border-red-400 bg-red-50 focus:ring-red-500' : 'border-gray-300'
              }`}
              required
            />
            {showUsernameError && (
              <p id="username-error" className="mt-1 text-sm text-red-600">{formErrors.username}</p>
            )}
          </div>
